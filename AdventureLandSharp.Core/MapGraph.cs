...
internal class MapGraphEdgeComparer : IEqualityComparer<IMapGraphEdge> {
    public bool Equals(IMapGraphEdge? lhs, IMapGraphEdge? rhs) => lhs != null && rhs != null &&
...
public class MapGraph {
    ...
    public List<IMapGraphEdge> InterMap_Djikstra(MapLocation start, MapLocation goal, MapGridHeuristic heuristic) {
        ...
        IMapGraphEdge[] startToGoal = start.Map == goal.Map ? start.Map.FindPath(start.Location, goal.Location, heuristic) : [];
        ...
        while (queue.TryDequeue(out MapLocation u, out float _)) {
            ...
            foreach (IMapGraphEdge edge in neighbors) {
                ...
                if (dist.TryGetValue(vLoc, out float prevAlt) && !(alt < prevAlt)) continue;
        ...
        if (!dist.TryGetValue(goal, out float _)) return [];
    ...
    private void AddEdge(IMapGraphEdge edge) {
        _edges.TryAdd(edge.Source, new(new MapGraphEdgeComparer()));
