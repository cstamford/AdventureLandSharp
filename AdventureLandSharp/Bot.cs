...
public class Bot
{
    ...
    public async Task Run()
    {
        ...
        if (!auth.Success) throw new Exception("Failed to login.");

        var serversAndCharacters = await _api.ServersAndCharactersAsync();

        var sessionSettings = serversAndCharacters.Characters
        ...
        {
            var socket = _socketFactory.CreateSocket(settings);
            await socket.Update(); 
            var character = CreateCharacterForRun(settings, socket);
        ...
        while (true)
        {
            ...
            var deltaTime = (float) now.Subtract(lastTick).TotalSeconds;
            lastTick = now;

            var runners = sessions.Select(n => RunOne(n.socket, n.character, deltaTime));
    ...
    {
        var cls = settings.Character.Type switch
        {
